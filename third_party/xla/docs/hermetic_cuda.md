...
## Local CUDA/CUDNN/NCCL installation

If required, you can use the local installation as a source of hermetic CUDA
rules. The following additional environment variables are required:

```
export LOCAL_CUDA_PATH=<local CUDA path>
export LOCAL_CUDNN_PATH=<local CUDNN path>
export LOCAL_NCCL_PATH=<local NCCL path>
```
...
export LOCAL_CUDNN_PATH="/usr/local/cudnn"
export LOCAL_NCCL_PATH="/usr/local/Downloads/dists/nvidia/nccl"
```
