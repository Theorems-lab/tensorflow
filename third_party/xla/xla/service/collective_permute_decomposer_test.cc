...
TEST_F(CollectivePermuteDecomposerTest, ForwardPipelineWithMatmul) {
  ...
  std::cout << transformed_module->ToString() << "\n";
  ...
  EXPECT_THAT(
      send1->ToString(),
      HasSubstr("_xla_send_recv_source_target_pairs=\"{{0,1},{1,2},{2,3}}\""));
  EXPECT_TRUE(CheckInstructionOrder(transformed_module, "recv", "send"));
  EXPECT_TRUE(CheckInstructionOrder(transformed_module, "recv", "recv-done"));
  EXPECT_TRUE(
      CheckInstructionOrder(transformed_module, "recv-done", "send-done"));
  EXPECT_TRUE(
      CheckInstructionOrder(transformed_module, "send-done", "send-done.1"));
}
